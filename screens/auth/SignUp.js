...
export default () => {
    ...
    const {data, loading,refetch,} = useQuery(CHECK_USERNAME,{variables:{username:username.value},skip:username.value==="",});
    const [createUserMutation] =useMutation(CREATE_USER,{variables:{username:username.value,nickname:nickname.value,gender:gender,location:location,birthDate:birthDate,machineId:constants.machineId}})
    const date = new Date();
    console.log(1);
    const onPressSignUp=()=>{
        console.log();
    ...
    useEffect(()=>{
        if(data!==undefined){
            if(data.checkUsername){
                setUsernameError({errorMessage:"available",errorStyle:{color:"green"}});
            }else{
                setUsernameError({errorMessage:"duplicated",errorStyle:{color:"red"}});
            }
        }
    ...
    return (
        ...
        <View >
            ...
            <Overlay
                ...
                width="auto"
                height="auto"
                onBackdropPress={()=>{setGenderOverlay(false); }}
            >
